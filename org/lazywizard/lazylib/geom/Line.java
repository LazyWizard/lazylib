...
/**
 * Implementation of a bunch of maths functions to do with lines. Note that lines
...
public class Line extends Shape
{
    ...
    public float distanceSquared(Vector2f point)
    {
        getClosestPoint(point, closest);
        //closest.sub(point);
        Vector2f.sub(closest, point, closest);
    ...
    public void getClosestPoint(Vector2f point, Vector2f result)
    {
        loc.set(point);
        //loc.sub(start);
        Vector2f.sub(loc, start, loc);

        //vec.dot(loc);
        float projDistance = Vector2f.dot(vec, loc);
