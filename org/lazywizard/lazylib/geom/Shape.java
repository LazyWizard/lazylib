...
public abstract class Shape implements Serializable
{
    ...
    public boolean intersects(Shape shape)
    {
        ...
        boolean result = false;
        float lPoints[] = getPoints();           // (x3, y3)  and (x4, y4)
        float thatPoints[] = shape.getPoints(); // (x1, y1)  and (x2, y2)
        int length = lPoints.length;
        ...
        for (int i = 0; i < length; i += 2)
        {
            ...
            for (int j = 0; j < thatLength; j += 2)
            {
                ...
                unknownA = (((lPoints[iNext] - lPoints[i]) * (double) (thatPoints[j + 1] - lPoints[i + 1]))
                        - ((lPoints[iNext + 1] - lPoints[i + 1]) * (thatPoints[j] - lPoints[i])))
                        / (((lPoints[iNext + 1] - lPoints[i + 1]) * (thatPoints[jNext] - thatPoints[j]))
                        - ((lPoints[iNext] - lPoints[i]) * (thatPoints[jNext + 1] - thatPoints[j + 1])));
                unknownB = (((thatPoints[jNext] - thatPoints[j]) * (double) (thatPoints[j + 1] - lPoints[i + 1]))
                        - ((thatPoints[jNext + 1] - thatPoints[j + 1]) * (thatPoints[j] - lPoints[i])))
                        / (((lPoints[iNext + 1] - lPoints[i + 1]) * (thatPoints[jNext] - thatPoints[j]))
                        - ((lPoints[iNext] - lPoints[i]) * (thatPoints[jNext + 1] - thatPoints[j + 1])));
